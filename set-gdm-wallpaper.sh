...
if [ "$1" = "--uninstall" ]; then
  ...
  if grep -q "wallpaper-gdm.png" /usr/share/gnome-shell/gdm3-theme.gresource; then
    cp -f /usr/share/gnome-shell/gdm3-theme.gresource.backup /usr/share/gnome-shell/gdm3-theme.gresource

    echo 'gdm3-theme.gresource recovered'
  fi
...
# Creating gdm3-theme.gresource.xml with theme file list and add header
echo '<?xml version="1.0" encoding="UTF-8"?>' >"$workdir/gdm3-theme.gresource.xml"
echo '<gresources><gresource>' >>"$workdir/gdm3-theme.gresource.xml"

for res_file in $(gresource list /usr/share/gnome-shell/gdm3-theme.gresource); do
  ...
  if [ "$res_file" != "/org/gnome/shell/theme/wallpaper-gdm.png" ]; then
    ...
    gresource extract /usr/share/gnome-shell/gdm3-theme.gresource "$res_file" >"$workdir$res_file"

    # add extracted file name to gdm3-theme.gresource.xml
    echo "<file>${res_file#\/}</file>" >>"$workdir/gdm3-theme.gresource.xml"
  fi
done

# add our image ($image) to theme path and to xml file
echo "<file>org/gnome/shell/theme/wallpaper-gdm.png</file>" >>"$workdir/gdm3-theme.gresource.xml"
...
# If wallpaper-gdm.png text inside gresource file, then this is modified file.
if ! grep -q "wallpaper-gdm.png" /usr/share/gnome-shell/gdm3-theme.gresource; then
  cp -f /usr/share/gnome-shell/gdm3-theme.gresource /usr/share/gnome-shell/gdm3-theme.gresource.backup
  echo "Backup"
fi

cp -f "$workdir/gdm3-theme.gresource" /usr/share/gnome-shell/

# Strange but safe from bug
rm -rf "$workdir/org"
rm -f "$workdir/gdm3-theme.gresource.xml"
rm -f "$workdir/gdm3-theme.gresource"
rm -r "$workdir"
